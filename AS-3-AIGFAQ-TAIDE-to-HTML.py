...
for name in SOURCES:
    ...
    for lang in LANGUAGES:
        if lang == "":
            ...
            html_name = name + '-QA-TAIDE.html'
        else:
            faq_name = name + '-QA-TAIDE_' + lang + '.csv'
            html_name = name + '-QA-TAIDE_' + lang + '.html'
        ...
        for index, row in df.iterrows():
            if row.category not in FAQ:
                FAQ[row.category] = {}
            FAQ[row.category]["["+str(row.title)+"] " + str(row.question)] = str(row.answer)
